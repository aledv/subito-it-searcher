...
def run_query(url, name, notify):
    ...
    product_list_items = soup.find_all('li', class_=re.compile(r'product-card'))
    msg = []

    for product in product_list_items:
        try:
            title = product.find('span', class_=re.compile(r'productList__itemTitle')).contents[0]
        except:
            title = "Unknown title"

        try:
            price = product.find('em', class_=re.compile(r'js-currentPriceValue')).contents[0]
        except:
            price = "Unknown price"

        try:
            link = 'brack.ch' + product.find('span', class_=re.compile(r'productList__itemTitle')).parent.get('href')
        except:
            link = "Unknown link"

        if not queries.get(name):   # insert the new search
            queries[name] = {url: {link: {'title': title, 'price': price, 'link': link}}}
            print("\nNew search added:", name)
            print("Adding result:", title, "-", price, "-", link)
        else:   # add search results to dictionary
            try:
                if not queries.get(name).get(url).get(link):   # found a new element
                    tmp = "New element found for "+name+": "+title+" @ "+price+" --> "+link+'\n'
                    print(tmp)
                    msg.append(tmp)
                    queries[name][url][link] = {'title': title, 'price': price, 'link': link}
            except:
